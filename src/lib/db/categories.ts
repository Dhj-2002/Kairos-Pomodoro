...
export async function deleteCategory(id: number): Promise<void> {
  const database = await getDb();
  // Tag deletion step 1: Revert reusable scheduling blocks to untagged gray.
  await database.execute(
    "UPDATE quick_blocks SET category_id = NULL WHERE category_id = $1",
    [id],
  );
  await database.execute(
    "UPDATE sequence_template_items SET category_id = NULL WHERE category_id = $1",
    [id],
  );

  // Tag deletion step 2: Preserve existing analytics behavior for sessions.
  await database.execute(
