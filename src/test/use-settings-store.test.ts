...
beforeEach(() => {
  ...
  useSettingsStore.setState({
    settings: {
      ...
      soundEnabled: true,
      scheduleReminderSound: "attention",
      scheduleReminderVolume: 50,
      theme: "system",
      themePreset: "sahara",
      timerStyle: "solid",
      miniWindowSize: "small",
      updateProxy: "",
    },
    loaded: false,
  });
...
describe("useSettingsStore", () => {
  describe("initial state", () => {
    it("has correct defaults", () => {
      ...
      expect(settings.miniWindowSize).toBe("small");
  ...
  describe("updateSetting", () => {
    ...
    it("persists the selected schedule reminder sound", async () => {
      ...
      await useSettingsStore
        .getState()
        .updateSetting("scheduleReminderSound", "digital");
      expect(setSetting).toHaveBeenCalledWith(
        "scheduleReminderSound",
        "digital",
      );
      expect(useSettingsStore.getState().settings.scheduleReminderSound).toBe(
        "digital",
      );
    });

    it("persists the schedule reminder volume", async () => {
      const { setSetting } = await import("@/lib/db");
      await useSettingsStore
        .getState()
        .updateSetting("scheduleReminderVolume", 75);
      expect(setSetting).toHaveBeenCalledWith("scheduleReminderVolume", "75");
      expect(useSettingsStore.getState().settings.scheduleReminderVolume).toBe(
        75,
      );
    });
  });
