...
describe("BackupFile shape contract", () => {
  it("a well-formed payload matches the expected structure", () => {
    ...
    expect(Object.keys(payload.data)).toHaveLength(13);
...
describe("resolveValue (restore NULL handling)", () => {
  ...
  it("restores the schedule reminder default for time_blocks", () => {
    ...
    expect(resolveValue("time_blocks", "notification_enabled", undefined)).toBe(1);
    // Present values pass through unchanged.
    expect(resolveValue("time_blocks", "completed", 1)).toBe(1);
  });

  it("restores required template defaults", () => {
    expect(resolveValue("schedule_templates", "name", null)).toBe("Untitled template");
    expect(resolveValue("schedule_templates", "color", undefined)).toBe("#c2652a");
    expect(resolveValue("template_blocks", "start_minute", null)).toBe(540);
    expect(resolveValue("template_blocks", "duration_minutes", undefined)).toBe(25);
    expect(resolveValue("template_blocks", "notification_enabled", null)).toBe(1);
    expect(resolveValue("template_blocks", "sort_order", undefined)).toBe(0);
  });

  it("restores required lightweight sequence defaults", () => {
    expect(resolveValue("quick_blocks", "name", null)).toBe("Untitled block");
    expect(resolveValue("quick_blocks", "duration_minutes", undefined)).toBe(30);
    expect(resolveValue("quick_blocks", "notification_enabled", null)).toBe(1);
    expect(resolveValue("sequence_templates", "name", null)).toBe("Untitled template");
    expect(resolveValue("sequence_template_items", "title", undefined)).toBe("Untitled block");
    expect(resolveValue("sequence_template_items", "duration_minutes", null)).toBe(30);
  });
